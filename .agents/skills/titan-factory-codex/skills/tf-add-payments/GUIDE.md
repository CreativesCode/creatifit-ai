...
Use the scope and corrections in SKILL.md first. These examples target the upstream stack; inspect actual dependency versions before applying. Shell code fences describe Bash recipes and must be translated for the active shell. TOOLKIT_ROOT means the installed plugin root, not the project root. No connection or execution is implied.
...
1. `$tf-add-login` ejecutado — busca `src/shared/lib/supabase/client.ts`. Si no existe, dile al usuario que ejecute `$tf-add-login` primero.
...
1. **Verificar pre-requisito:** Busca `src/shared/lib/supabase/client.ts`. Si no existe, dile al usuario: "Ejecuta $tf-add-login primero."
