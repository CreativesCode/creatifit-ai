> Compatibility: this preserved recipe targets the upstream AI SDK v5/OpenRouter approach. The skill entrypoint governs provider choice and scope. Check installed versions before adapting code; do not treat this reference as a request to migrate providers.
...
```bash
# Dependencias
# Resolve compatible versions for this v5 recipe against the existing project lockfile; do not install an unbounded latest SDK.
