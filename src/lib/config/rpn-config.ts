...
export const RPN_CONFIG = {
  ...
  gender: {
    ...
    female: {
      focus_areas: ["upper_body_strength", "core_stability", "lower_body_power"],
      common_weaknesses: ["upper_body_strength", "grip_strength", "bone_density"],
      recommended_exercises: ["push_ups", "rows", "lunges", "glute_bridges"]
    },
    other: {
      focus_areas: ["balanced_development", "functional_movement", "overall_fitness"],
      common_weaknesses: ["general_fitness", "movement_patterns", "consistency"],
      recommended_exercises: ["bodyweight_exercises", "functional_movements", "mobility_work"]
    }
  },
...
export function getRPNConfig(userProfile: {
  ...
  const levelConfig = RPN_CONFIG.level[userProfile.level as keyof typeof RPN_CONFIG.level];
  const objectiveConfig = RPN_CONFIG.objective[userProfile.objective as keyof typeof RPN_CONFIG.objective];
  const genderConfig = RPN_CONFIG.gender[userProfile.gender as keyof typeof RPN_CONFIG.gender];
